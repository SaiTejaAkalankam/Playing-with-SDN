...
def _handle_flowstats_received(event):
	global src_dpid,dst_dpid,input_pkts1,output_pkts1,input_pkts2,output_pkts2

	for f in event.stats:                                        
		if f.match.dl_type==0x0800 and f.match.nw_proto==6 and f.match.tp_dst==80 and (f.match.nw_dst==IPAddr("10.0.0.4") or f.match.nw_dst == IPAddr("10.0.0.1")) and event.connection.dpid==src_dpid:
			input_pkts1 = f.packet_count

		if f.match.dl_type==0x0800 and f.match.nw_proto==6 and f.match.tp_dst==80 and (f.match.nw_dst==IPAddr("10.0.0.4") or f.match.nw_dst == IPAddr("10.0.0.1")) and event.connection.dpid==dst_dpid:
			output_pkts1 = f.packet_count

			if input_pkts1 !=0:
				print(getTheTime(),"Packet Loss Count for HTTP traffic H4-H1=", (input_pkts1 - output_pkts1))


		if f.match.dl_type==0x0800 and f.match.nw_dst==IPAddr("10.0.0.1") and event.connection.dpid==src_dpid:   
			input_pkts2 = f.packet_count
	 
		if f.match.dl_type==0x0800 and f.match.nw_dst==IPAddr("10.0.0.1") and event.connection.dpid==dst_dpid:   
			output_pkts2 = f.packet_count

			if input_pkts2 != 0:
				print(getTheTime(),"Packet Loss Count for non-HTTP traffic H4-H1 =", (input_pkts2 - output_pkts2))
...
def _handle_PacketIn(event):
	...
	if dpid==s1_dpid:
		...
		msg.idle_timeout = 100
		msg.hard_timeout = 200
		msg.match.dl_type = 0x0800               # we have to follow 'NORMAL FORM' in constructing the flow, when specifying about 
		msg.match.tp_dst = 80                    # http traffic-- 'port 80', we have to specify using TCP protocol--nw_proto 6 
		msg.match.nw_proto = 6                   # then below IP layer addresses     
		msg.match.nw_src = "10.0.0.1"        
